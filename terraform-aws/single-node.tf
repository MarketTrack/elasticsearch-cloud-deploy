...
resource "aws_launch_configuration" "single_node" {
  ...
  security_groups = ["${concat(list(aws_security_group.elasticsearch_security_group.id), list(aws_security_group.elasticsearch_clients_security_group.id), var.additional_security_groups, var.writer_security_groups)}"]
